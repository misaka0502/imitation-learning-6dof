...
def get_runs(args: argparse.Namespace) -> List[Run]:
    ...
    if args.sweep_id:
        ...
        runs: List[Run] = list(api.runs(f"yumio-harbin-institute-of-technology-shenzhen/{args.project_id}"))
    else:
    ...
    for run in runs:
        print(run.config.get("control", {}).get("control_mode", "delta"))
    ...
    try:
        while True:
            ...
            for run in runs:
                ...
                print(f"control mode: {config.control.control_mode}")
