...
class DataCollectorAugmentor:
    ...
    def __init__(
        ...
        code_dir = "/home2/zxp/Projects/Juicer_ws/imitation-juicer/src/data_collection"
        mesh_file = '/home2/zxp/Projects/Juicer_ws/imitation-juicer/foundationpose/demo_data/square_table_leg/mesh/square_table_leg.obj'
        test_scene_dir = '/home2/zxp/Projects/Juicer_ws/imitation-juicer/foundationpose/demo_data/square_table_leg'
        self.debug_dir = '/home2/zxp/Projects/Juicer_ws/imitation-juicer/src/data_collection/debug'
        self.debug = 0
        set_logging_format()
        set_seed(0)
        mesh = trimesh.load(mesh_file, force='mesh')
        os.system(f'rm -rf {self.debug_dir}/rollouts_vis/* {self.debug_dir}/rollouts_ob/* && mkdir -p {self.debug_dir}/rollouts_vis {self.debug_dir}/rollouts_ob')
        self.obj_to_origin, extents = trimesh.bounds.oriented_bounds(mesh)
        self.obj_bbox = np.stack([-extents/2, extents/2], axis=0).reshape(2,3)
        scorer = ScorePredictor()
        refiner = PoseRefinePredictor()
        glctx = dr.RasterizeCudaContext()
        self.est = FoundationPose(model_pts=mesh.vertices, model_normals=mesh.vertex_normals, mesh=mesh, scorer=scorer, refiner=refiner, debug_dir=self.debug_dir, debug=self.debug, glctx=glctx)
        logging.info("estimator initialization done")
        self.reader = YcbineoatReader(video_dir=test_scene_dir, shorter_side=None, zfar=np.inf)
        self.eps_reset = True
        self.eps = 0
        self.ref_color = cv2.imread("/home2/zxp/Projects/FoundationPose/demo_data/square_table_leg/rgb/0000000000000000000.png")[...,:3]
        self.red_color = cv2.resize(self.ref_color, (self.reader.W,self.reader.H), interpolation=cv2.INTER_NEAREST)
        self.ref_depth = cv2.imread("/home2/zxp/Projects/FoundationPose/demo_data/square_table_leg/depth/0000000000000000000.png",-1)/1e3
        self.ref_depth = cv2.resize(self.ref_depth, (self.reader.W,self.reader.H), interpolation=cv2.INTER_NEAREST)
        self.ref_depth[(self.ref_depth<0.001) | (self.ref_depth>=self.reader.zfar)] = 0
    ...
    def store_transition(
        ...
        with suppress_all_output(True):
            ...
            color = self.reader.get_color(color_img)
            depth_img = obs["depth_image2"].squeeze(0).cpu().numpy() * 1000
            depth_img = depth_img.astype(np.uint16)
            depth_img = 65535 - depth_img
            depth = self.reader.get_depth(depth_img)
            if self.eps_reset:
                self.eps_reset = False
                mask = self.reader.get_mask(0).astype(bool)
                pose = self.est.register(K=self.reader.K, rgb=self.ref_color, depth=self.ref_depth, ob_mask=mask, iteration=5)
                pose = self.est.track_one(rgb=color, depth=depth, K=self.reader.K, iteration=2)
                center_pose = pose@np.linalg.inv(self.obj_to_origin)
                self.vis_begin = draw_posed_3d_box(self.reader.K, img=color, ob_in_cam=center_pose, bbox=self.obj_bbox)
                self.vis_begin = draw_xyz_axis(color, ob_in_cam=center_pose, scale=0.1, K=self.reader.K, thickness=3, transparency=0, is_input_rgb=True)
                cv2.imwrite(f'{self.debug_dir}/rollouts_vis/{self.eps:019d}_begin.png', self.vis_begin)
                # np.savetxt(f'{self.debug_dir}/rollouts_ob/{self.eps:019d}_begin.txt', pose.reshape(4,4))
            else:
                pose = self.est.track_one(rgb=color, depth=depth, K=self.reader.K, iteration=2)
        ...
        n_ob["6dof_poses"] = torch.from_numpy(pose).cpu()
